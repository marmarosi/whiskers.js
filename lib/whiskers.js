...
(function(whiskers) {
  ...
  whiskers.compile = function(template) {
    ...
    // replace tags
    template = template.replace(/(\\*){(?:([\w_.\-]+)|([\w_.]+\(.+\))|>([\w_.\-]+)|for +([\w_]+) +in +([\w_.\-]+)|if +(not +|)([\w_.\-]+)|\/(for|if))}/g, function(str, escapeChar, key, extFn, partial, iterVar, forKey, ifNot, ifKey, closeStatement, offset, s) {
      if (escapeChar) return str.replace('\\\\', '');
      // {foo}
      if (key) {
        // {else}
